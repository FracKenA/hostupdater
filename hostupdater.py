...
# This script is used for updating hosts
...
def json_builder(data, host):
    hostgroup, address, host = host.split(",")
    data["host_name"] = host
    data["address"] = address
    data["hostgroups"].append(hostgroup)
...
def main():
    ...
    logging.basicConfig(
        ...
        level=logging.INFO,
        filename="hostupdater.log"
    )
    logger = logging.getLogger('hostupdater')
    ...
    with open(args.hostsfile, 'r') as importfile:
        for host in importfile:
            ...
            host_exact = "/".join([api_config_host, host.split(",")[2]])
            print(host)
            print(host_exact)

            # Updates the host objects with the new hostgroup.
            # TODO: Move patches into generic post function.
            r = requests.patch(
                api_config_host,
                data=json_payload,
                verify=ssl_check,
